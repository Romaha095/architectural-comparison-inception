...
# Zamrznutá štruktúra -> uči sa len klasifikačná hlavička
#for layer in base_model.layers:
#    layer.trainable = False

# Odmrazená polovica modelu -> uči sa polovica vrstiev modelu od konca + klasifikcačna hlava
#for layer in base_model.layers[-(len(base_model.layers)//2):]:
#    layer.trainable = True

# Odmrazené všetky vrstvy -> učí sa celý model
...
history_finetune = model.fit(x_train, y_train, validation_data=(x_val, y_val),
                             epochs=200, batch_size=BATCH_SIZE, callbacks=[early_stopping])
