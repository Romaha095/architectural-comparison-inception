...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate trained checkpoint on IDC dataset using metrics.py"
    )
    parser.add_argument(
        "--config_path",
        type=str,
        default="configs/first_results_idc_inception_v3.json",
        help="Path to JSON config file used for training",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        required=True,
        help="Path to .pt checkpoint file (with model_state)",
    )
    parser.add_argument(
        "--split",
        type=str,
...
def main():
    ...
    logger.info(
        "Evaluating trained %s checkpoint on %s split",
        model_cfg["arch"],
        args.split,
    )
